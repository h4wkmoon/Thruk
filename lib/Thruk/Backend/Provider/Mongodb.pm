...
sub new {
    ...
    my $self = {
        'mongo'       => MongoDB::Connection->new(host => $db),
        'dbname'      => $dbname,
        'config'      => $config,
...
sub get_processinfo {
    ...
    my $data = {
        $self->peer_key() => $self->{'db'}->processinfo
                                  ->find_one()
    };
    $data->{$self->peer_key()}->{'data_source_version'} = "Mongodb ".$data->{$self->peer_key()}->{'data_source_version'};
...
sub get_host_stats {
    ...
    my $result = $self->{'db'}->run_command($cmd);
